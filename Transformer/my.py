...
# ]
src_vocab = {'PAD':0,'BOS':1,'EOS':2,'1':3,'2':4,'3':5}
idx2word = {}
for i in src_vocab:
    idx2word[src_vocab[i]] = i
tgt_vocab = src_vocab
...
def data_generator():
    ...
    for i in range(2000):
        ...
        kernel_input = [src_vocab[str(random.randint(1,3))] for i in range(random_length)]
    ...
    return torch.LongTensor(enc_inputs),torch.LongTensor(dec_inputs),torch.LongTensor(dec_targets)

enc_inputs, dec_inputs, dec_outputs = data_generator()
...
loader = Data.DataLoader(MyDataSet(enc_inputs, dec_inputs, dec_outputs), 16, True)
...
for epoch in range(10):
    index = 0
    loss_sum = 0
    pbar = ProgressBar(n_total=len(loader), desc='Training')
    print("Epoch: ",epoch)
